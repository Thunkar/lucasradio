...
PN532_I2C_CUSTOM::PN532_I2C_CUSTOM(TwoWire &wire, uint8_t sda, uint8_t scl)
{
    _wire = &wire;
    command = 0;
    _sda = sda;
...
void PN532_I2C_CUSTOM::begin()
{   
    _wire->setClock(400000);
    _wire->begin(_sda, _scl);
}

void PN532_I2C_CUSTOM::wakeup()
{
    delay(500); // wait for all ready to manipulate pn532
}

int8_t PN532_I2C_CUSTOM::writeCommand(const uint8_t *header, uint8_t hlen, const uint8_t *body, uint8_t blen)
{
    command = header[0];
...
int16_t PN532_I2C_CUSTOM::getResponseLength(uint8_t buf[], uint8_t len, uint16_t timeout)
{
...
int16_t PN532_I2C_CUSTOM::readResponse(uint8_t buf[], uint8_t len, uint16_t timeout)
{
    uint16_t time = 0;
    uint8_t length;
...
int8_t PN532_I2C_CUSTOM::readAckFrame()
{
